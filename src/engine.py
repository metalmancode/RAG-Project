...
def _create_new_vector_store(
    ...
    Explanation:
    - SimpleDirectoryReader: Automatically finds and reads all supported files from the given directory.
    - SentenceSplitter: Breaks the long document into smaller, more manageable chunks (based on CHUNK_SIZE/CHUNK_OVERLAP).
    - VectorStoreIndex.from_documents: Orchestrates the creation of the vector store by using the embed_model to create vectors.
    - .persist(): Saves the result to disk to avoid rebuilding it.
    """
...
def get_vector_store(embed_model: HuggingFaceEmbedding) -> VectorStoreIndex:
    ...
    Explanation:
    - We use exist_ok=True to ensure the directory exists safely.
    - any(VECTOR_STORE_PATH.iterdir()): A clever way to check if there are any files inside the folder.
    - load_index_from_storage: Loads the persisted index from disk. MUST pass the exact same embed_model.
    """
...
def get_chat_engine(
    ...
    Explanation:
    - vector_index.as_chat_engine: A high-level LlamaIndex method that builds a chat engine directly from a vector index.
    """
